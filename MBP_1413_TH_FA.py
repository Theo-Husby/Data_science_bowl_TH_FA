...
img_size = (256, 256)
# TODO: Try changing the batch size. How does it affect how fast the model trains?
# TODO: Try changing the epochs. How does it affect the convergence of the model?
batch_size = 20;
epochs = 60;
steps_per_epoch = 200
...
def load_dataset(fpath, img_size):
    ...
    for ids in data_ids:
        ...
        #data[ids].append(masks)
        data[ids].append(masks.astype(int)) #Test integer masks
...
#train_images, test_images, train_masks, test_masks = train_test_split(image_set, mask_set, test_size=0.1, random_state=27) 
...
def unet_model2(Image_Size):
    # Idea comes from Trevor Nell's tutorial. 
    ...
    p1 = MaxPool2D()(c1) #Double check this line if code doesn't run.
    ...
    p2 = MaxPool2D()(c2) #Double check this line if code doesn't run.
    ...
    p3 = MaxPool2D()(c3) #Double check this line if code doesn't run.
    ...
    p4 = MaxPool2D()(c4) #Double check this line if code doesn't run.
    ...
    u6 = Concatenate()([u6, c4]) #why is concatenate used here. Doesn't that mess up the activation layer dimensions?
    ...
    u7 = Concatenate()([u7, c3]) #why is concatenate used here. Doesn't that mess up the activation layer dimensions?
    ...
    u8 = Concatenate()([u8, c2]) #why is concatenate used here. Doesn't that mess up the activation layer dimensions?
    ...
    u9 = Concatenate()([u9, c1]) #why is concatenate used here. Doesn't that mess up the activation layer dimensions?
...
# Experimenting with other loss functions.
# Needs to be a Keras loss class.
...
'''MODEL FITTING'''
# TODO: Try changing the learning rate. How does it affect the convergence of the model?
learning_rate = 0.001
...
if data_augmentation:
    #history = model.fit(data_augmenter.flow(train_images, train_masks, batch_size=batch_size),
    #                batch_size=batch_size, epochs=epochs, validation_data=(test_images, test_masks))
    #history = model.fit(x=image_data_augmenter.flow(train_images, batch_size=batch_size, seed=data_augmenter_seed),
    #                    y=mask_data_augmenter.flow(train_masks, batch_size=batch_size, seed=data_augmenter_seed),
    #                    batch_size=batch_size, epochs=epochs, validation_data=(test_images, test_masks))
